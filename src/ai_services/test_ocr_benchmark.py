...
def run_benchmark():
    ...
    if False:
        ...
        if not os.path.exists(test_images_directory):
            ...
            logger.error(
                "Please create it and place your test images with QR/DataMatrix codes inside."
            )
            return
        ...
        for image_file in image_files:
            ...
            result = ocr_processor.find_validated_code(
                ...
                on_timeout=lambda: logger.warning(
                    f"❌ FAILURE: Timeout occurred after {timeout_sec} seconds."
                ),
            )
            ...
            if result["status"] == "success":
                logger.info(
                    f"✅ SUCCESS: Valid code '{result['code']}' found and validated."
                )
                logger.info(f"Total time for {image_file}: {total_time:.4f} seconds.")
            else:
                logger.warning("❌ FAILURE: No valid codes were found in this image.")
                logger.info(
                    f"Total time for {image_file}: {total_time:.4f} seconds (no codes found)."
                )
    ...
    if True:
        ...
        if result["status"] == "success":
            ...
            logger.warning("❌ FAILURE: No valid codes were found in this image.")
            logger.info(
                f"Total time for Camera {0}: {total_time:.4f} seconds (no codes found)."
            )
