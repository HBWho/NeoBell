...
class FaceProcessing:
    ...
    def start_background_recording(self, camera_id: int, output_file: str):
        ...
        self.recording_thread = threading.Thread(
            target=self._record_video_thread, args=(camera_id,)
        )
    ...
    def process_results(self, dfs: list) -> tuple[bool, str | None]:
        ...
        logger.info(
            f"Best Match Found: User ID {best_match_user_id} (Distance: {best_match_distance:.3f})"
        )
    ...
    def recognize_face(self, camera_id: int, db_path: str) -> tuple[bool, str | None]:
        ...
        try:
            ...
            logger.error(
                "An unexpected error occurred during face recognition.", exc_info=True
            )
            return False, None
        finally:
