...
class TTSService:
    def __init__(self, engine_path=None, lang="en"):
        ...
        logger.info(f"TTSService initialized. Engine: {self.executable}, Lang: {self.lang_code}")
    ...
    def speak(self, text_to_say):
        ...
        try:
            ...
            logger.error(f"ERROR TTS: Command '{command[0]}' not found. Ensure eSpeak NG is installed and in PATH, or engine_path is set.")
        except subprocess.CalledProcessError as e:
            # In case of error, we might want to see stderr
            # For now, just a generic message. To debug, remove DEVNULL for stderr.
            logger.error(f"ERROR TTS: eSpeak NG failed. Return code: {e.returncode}")
