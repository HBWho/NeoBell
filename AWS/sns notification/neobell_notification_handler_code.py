...
DEVICE_USER_LINK_TABLE_NAME = os.environ.get('DYNAMODB_DEVICE_USER_LINK_TABLE_NAME', 'DeviceUserLink')

users_table = DYNAMODB_RESOURCE.Table(NEOBELL_USERS_TABLE_NAME , 'NeoBellUsers')
...
def _send_push_notification(user_id_to_notify, notification_type, message_payload_details):
    ...
    try:
        ...
        if not item or 'sns_android_endpoint_arn' not in item:
        ...
        sns_endpoint_arn = item['sns_android_endpoint_arn']
...
def lambda_handler(event, context):
    ...
    if not (user_id_direct or sbc_id_fan_out) or not message_payload:
        logger.error("Missing required fields: (user_id OR sbc_id) AND notification_type AND message_payload.")
